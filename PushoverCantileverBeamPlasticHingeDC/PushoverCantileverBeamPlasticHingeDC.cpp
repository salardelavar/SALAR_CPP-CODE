...
void ElementInternalForce(double ,double ,double ,double ,double [],double [],double [],double );// Calculate internal element force
...
void MessageCheckInputDataNegative(double ,double ,double ,int ,double ,int );
void MessageCheckInputPlasticHingeNegative(double [],double [],int );
...
int main(){   
	system("color 0A");
     MessageFileExist01();
     MessageFileExist02();
    double L,EI,Dini,Dmax,tolerance,residual,applied_load[4];
    int itermax,M,Y;
    IMPORT_DATA01(L,EI,applied_load,Dini,Dmax,M,itermax,tolerance);
    MessageCheckInputDataNegative(L,EI,Dmax,itermax,tolerance,M);
    
    double TET[5],MOM[5];

    IMPORT_DATA02(TET,MOM,Y);
	MessageCheckInputPlasticHingeNegative(TET,MOM,Y);
...
void ElementInternalForce(double A,double B,double C,double D,double U[],double E[],double G[],double Z){
...
cout<<"\t\t\t\t"<<Qf<<"   Nonlinear Analysis of 2D cantilever Beam with Displacement Control  "<<Qf<<"\n";
cout<<"\t\t\t\t"<<Qg;
for (i=1;i<72;i++)
{cout<<Qb;}
cout<<Qk<<endl;
...
cout<<Qe<<endl;
    ...
    cout<<"      Length :                                                                 "<<L<<endl;
    cout<<"      Flextural rigidity:                                                      "<<EI<<endl;
...
void MessageFileExist01(){
	ifstream thefile2("PushoverCantileverBeamPlasticHingeDC-inputDATA.csv");
	if (!thefile2){
	MessageErrorReportTEXT();
	cout<<"          File is not available! -> [ PushoverCantileverBeamPlasticHingeDC-inputDATA.csv ]"<<endl;;
	Sleep(40000);
	exit(1);
	}
}
void MessageFileExist02(){
	ifstream thefile2("PushoverCantileverBeamPlasticHingeDC-inputHINGE.csv");
	if (!thefile2){
	MessageErrorReportTEXT();
	cout<<"          File is not available! -> [ PushoverCantileverBeamPlasticHingeDC-inputHINGE.csv ]"<<endl;;
	Sleep(40000);
	exit(1);
	}
}
...
void MessageCheckInputDataNegative(double L,double EI,double Dmax,int itermax,double tolerance,int M){
	if ( L < 0 ||  EI < 0 ||  Dmax < 0 || itermax < 0 || tolerance< 0 ){
    MessageErrorReportTEXT();
    cout<<"               Please check this file! -> [ PushoverCantileverBeamPlasticHingeDC-inputDATA.csv ]"<<endl;
...
void MessageCheckInputPlasticHingeNegative(double A[],double B[],int n){
	int i;
	for(i=0;i<n;i++){
	if (A[i] < 0|| B[i] < 0){			
    MessageErrorReportTEXT();
    cout<<"               Please check this file! -> [ PushoverCantileverBeamPlasticHingeDC-inputHINGE.csv ]"<<endl;
    ...
    if (A[i] <= A[i-1]){
    MessageErrorReportTEXT();
	printf("          Please check the input file! -> [ PushoverCantileverBeamPlasticHingeDC-inputHINGE.csv ]\n");
...
// MATLAB OUTPUT
int i;
ofstream OUTm;OUTm.open("PushoverCantileverBeamPlasticHingeDC-outputMATLAB.m");
OUTm<<" %  Nonlinear Analysis of 2D cantilever Beam with Displacement Control  %\n"<<endl;
OUTm<<"disp_Dof2=["<<endl;
for(i=0;i<n;i++)
OUTm<<A[i][0]<<endl;
OUTm<<"];"<<endl<<endl;
...
// EXCEL OUTPUT
int i;
ofstream OUTe;OUTe.open("PushoverCantileverBeamPlasticHingeDC-outputEXCEL.csv");
OUTe<<" ###  Nonlinear Analysis of 2D cantilever Beam with Displacement Control  ###\n"<<endl;
OUTe<<"Increment,Base Shear[DOF(1)],Base Moment[DOF(2)],Rotation [DOF(2)],Displacement [DOF(3)],Rotation [DOF(4)]"<<endl;
for(i=0;i<n;i++)
OUTe<<i+1<<","<<B[i][0]<<","<<B[i][1]<<","<<A[i][0]<<","<<A[i][2]<<","<<A[i][1]<<endl;
OUTe.close();
}
...
void IMPORT_DATA01(double &L,double &EI,double applied_load[],double &Dini,double &Dmax,int &M,int &itermax,double &tolerance){
	ifstream IN1;IN1.open("PushoverCantileverBeamPlasticHingeDC-inputDATA.csv");
...
void IMPORT_DATA02(double TET[],double MOM[],int &k){
	...
	int Y=0;
	char CHAR;
	ifstream IN2("PushoverCantileverBeamPlasticHingeDC-inputHINGE.csv");//import strain-stress of elements
...
void ANALYSIS(double TET[],double MOM[],double L,double EI,double applied_load[],double Dini,double Dmax,int itermax,double tolerance,int Y,int M){
    ...
    double residual,Rk[5],Krk,K[3][3],eleF[4],Kinit[NN][NN],Kt[NN][NN],InvKinit[NN][NN];
	std::clock_t start;start = clock();
    double duration;
    
    double AA,BB,CC,DD,up,m,Sum,Dx,ff,f[NN],F[NN],Fi[NN],u[NN],du[NN],DU[N],II[N],IT[N],output_u[N][3],output_base[N][2];
    ...
    zMAX = z+1;
    ElementInternalForce(AA,BB,CC,DD,u,eleF,applied_load,up);
    	for (i=0;i<NN;i++)
		output_u[z][i]=u[i];//output displacement
		output_u[z][2]=up;//output incremental displacement
		output_base[z][0]= eleF[0] - applied_load[2];//output base shear
		output_base[z][1]= eleF[1] - applied_load[3];//output base moment
	    DU[z]=residual;II[z]=z;IT[z]=it;
    //cout<<"disp: "<<0<<"   "<<u[0]<<"   "<<u[1]<<"   "<<u[2]<<endl;
	//cout<<"eleF: "<<eleF[0]<<"   "<<eleF[1]<<"   "<<eleF[2]<<"   "<<eleF[3]<<endl;
    ...
    */	
		...
		cout<<"\n\a - Output data is written in Excel and Matlab file -"<<endl; 
