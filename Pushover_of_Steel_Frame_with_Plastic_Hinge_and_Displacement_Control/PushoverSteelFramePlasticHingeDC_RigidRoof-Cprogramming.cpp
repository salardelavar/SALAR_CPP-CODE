...
#define ShowText01 "PushoverSteelFramePlasticHingeDC_RigidRoof-inputDATA.csv"
#define ShowText02 "PushoverSteelFramePlasticHingeDC_RigidRoof-inputCOORDINATE.csv"
#define ShowText03 "PushoverSteelFramePlasticHingeDC_RigidRoof-inputHINGE.csv"
#define ShowText04 "Output data is written in Excel and Html file"
#define ShowText05 "PushoverSteelFramePlasticHingeDC_RigidRoof-outputEXCEL.csv"
#define ShowText06 "PushoverSteelFramePlasticHingeDC_RigidRoof-outputHTML.html"
...
#define ShowText10 "             with Plastic Hinge at Column Supports         "
...
void MessageHingeTEXT(double TET01[],double MOM01[],int n);
...
void HingeStiffnessCOFF(double A[],double B[],double C[],int n);
void HingeStiffness(double My,double A[],double B[],double C[],double &MomS,double &KRK,int n);
...
int main(){
    ...
    MessageHingeTEXT(TET01,MOM01,np);
    textcolor(11);
...
void IMPORT_DATA03(double TET01[],double MOM01[],int &k){
	...
	//printf("%d - TET01[%d]: %lf - MOM01[%d]: %lf");
	i++;
	}
...
void ANALYSIS(double TET01[],double MOM01[],int np,double EI01,double EA01,double Dini,double x01[],double y01[],int M){
   ...
   HingeStiffnessCOFF(TET01,MOM01,Rk01,np);

    L[0] = SQRT2((x01[1]-x01[0])*(x01[1]-x01[0])+(y01[1]-y01[0])*(y01[1]-y01[0]));
    printf("\t ================================= \n");
	printf("\t  Increment    Disp     BaseShear  \n");
	printf("\t ================================= \n");
	double My,A,B,C,d; // Initial Condition for Internal moment in node 1
	My = 0;// Intial Yield Moment is Zero
	A = 12*EI01/(L[0]*L[0]*L[0]);
	B = 6*EI01/(L[0]*L[0]);
	C = 4*EI01/(L[0]);
	  for (z=0;z<M;z++){
	  u[z] = Dini*(z+1);
	  ...
	  if (ABS(My) < MOM01[0]){	
	  F_BaseShear[z] = -A*u[z];
	  My = -B*u[z];// Moment in Hinge 1
	  RO = 0.0;// Plastic Hinge Rotation is Zero
	  d = u[z];
      }
	  if (ABS(My) >= MOM01[0]){// Check for Plastic Hinge
	  HingeStiffness(My,TET01,MOM01,Rk01,MomS01,krk01,np);
	  RO = (-MomS01+B*u[z])/(C+krk01);// Rotation in Hinge 1
	  RO += (u[z]-d)/L[0];//Chord Rotation of Hinge after Yield Rotation
	  F_BaseShear[z] = -A*u[z] + B*RO;
      My = -B*u[z] + C*RO;// Moment in Hinge 1
	  }
      ...
      printf("\t Rotation at Hinge [%.3e] reached to Ultimate Rotation [%.3e]\n",RO,TET01[np-1]);
	  break;
	  }
